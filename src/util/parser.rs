...
use crate::{
    ...
    commands::{
        command_req::{CommandType, ReqHeader, ReqMetaData, ReqType},
        command_res::ResType,
    },
};
...
impl ArrType {
    fn from(value: u8) -> Result<Self, Error> {
        match value {
            1 => Ok(ArrType::I8),
            2 => Ok(ArrType::I16),
            4 => Ok(ArrType::I32),
            ...
            15 => Ok(ArrType::U64),
            23 => Ok(ArrType::Txt),
            _ => Err(Error::new(ErrorKind::InvalidData, "invalid Arr type")),
        }
    }
}
    ...
    R: AsyncBufRead + Unpin,
{
    ...
    // let mut crlf = [0u8; 2];
    // reader.read_exact(&mut crlf).await?; //skip '\r\n\'

    let mut buf = vec![0u8; len as usize];
    reader.read_exact(&mut buf).await?; //read till len chars

    // Consume the mandatory trailing CRLF
    // let mut crlf = [0u8; 2];
    // reader.read_exact(&mut crlf).await?; //skip '\r\n'
...
pub async fn read_req_header<R>(reader: &mut R) -> io::Result<ReqHeader>
where
    R: AsyncBufRead + Unpin,
{
    //\n\n<u8><u8><[u8:16]><i64>\n\n
    //\n\n<version><req_type><pool_id><timestamp>\n\n
    //\n\n<1><command_req|replica_req>\n\n
    //command_req: 1
    //replica_req: 2
    //
    let mut buf = [0u8; 2];
    ...
    let req_type_byte = reader.read_u8().await?;
    let mut meta_data: Option<ReqMetaData> = None;
    tracing::debug!("req type byte: {:?}, {:?}", version, req_type_byte);
    ...
    if req_type == ReqType::Client || req_type == ReqType::Replica {
        tracing::debug!("read header");
        let mut uuid_buf = [0u8; 16];

        reader.read_exact(&mut uuid_buf).await?;
        tracing::debug!("uuid bytes: {:?}", uuid_buf);
        let client_id = uuid::Uuid::from_bytes(uuid_buf);
        tracing::debug!("uuid : {:?}", client_id);
        let hash = reader.read_u64().await?;
        //    tracing::debug!("hash : {:?}", hash);
        let timestamp = reader.read_i64().await?;
        tracing::debug!("timestamp :{:?}", timestamp);
        meta_data = Some(ReqMetaData {
            client_id: client_id,
            hash: hash,
            server_meta_last_fetch_time: timestamp,
        });
    }

    let command_type_byte = reader.read_u8().await?;
    let command_type = match command_type_byte {
        1 => CommandType::Cfg,
        2 => CommandType::Data,
    ...
    tracing::debug!("req header: {:?}", req_header);
    Ok(req_header)
}
    ...
    R: AsyncBufRead + Unpin,
{
    ...
    if has_cluster_info != 0 {
        ...
        // tracing::debug!("CLUSTER: {:?}", cluster_str);
        cluster = Some(
...
pub async fn read_server_res_header<R>(reader: &mut R) -> io::Result<()>
where
    R: AsyncBufRead + Unpin,
{
    //log::info!("reading res header");
...
pub fn append_small_string(buf: &mut Vec<u8>, data: &str) {
    ...
    // The wire format uses a single u8 length prefix; silently truncating that
    // prefix while still writing the full payload desyncs every field the
    // receiver reads afterwards, so fail loudly instead.
    assert!(
...
pub async fn read_i16_array<T, R>(reader: &mut R) -> io::Result<Vec<i16>>
where
    R: AsyncBufRead + Unpin,
{
    ...
    // let num_byte = reader.read_u8().await?;
    // if num_byte != b'@' {
    //     return Err(io::Error::new(
    //         io::ErrorKind::InvalidData,
    //         "Expected u8 prefix @ ",
...
pub async fn read_i32_array<T, R>(reader: &mut R) -> io::Result<Vec<i32>>
where
    R: AsyncBufRead + Unpin,
{
    let arr_byte = reader.read_u8().await?;
...
pub async fn read_u8_array<T, R>(reader: &mut R) -> io::Result<Vec<u8>>
where
    R: AsyncBufRead + Unpin,
{
    let arr_byte = reader.read_u8().await?;
...
pub async fn read_u16_array<T, R>(reader: &mut R) -> io::Result<Vec<u16>>
where
    R: AsyncBufRead + Unpin,
{
    let arr_byte = reader.read_u8().await?;
...
pub async fn read_u32_array<T, R>(reader: &mut R) -> io::Result<Vec<u32>>
where
    R: AsyncBufRead + Unpin,
{
    let arr_byte = reader.read_u8().await?;
...
pub fn append_req_header(
    ...
    // tracing::debug!(
    //     "HEADER: HASH {:?}, client_id: {:?}, timestamp: {:?} ",
    //     hash, client_uuid, timestamp
    // );
    buf.extend_from_slice(b"\n\n");
    let v = 1 as u8;
    //version 1
...
pub fn append_server_req_header(buf: &mut Vec<u8>) {
    buf.extend_from_slice(b"\n\n");
    let v = 1 as u8;
    //version 1
    buf.extend_from_slice(&v.to_be_bytes());
...
// pub fn append_res_header(buf: &mut Vec<u8>, version: u8, res_type: ResType, cluster_updated: bool) {
//     buf.extend_from_slice(b"\n\n");
//     buf.extend_from_slice(&version.to_be_bytes());

//     if cluster_updated {
//         let cluster_info = 1 as u8;
//         buf.extend_from_slice(&cluster_info.to_be_bytes());
//         let cluster_list = Cluster::get_cluster_dto();
//         let s_l = serde_json::to_string(&cluster_list).unwrap();
//         append_bulk_string(buf, &s_l);
//     } else {
//         let cluster_info = 0 as u8;
//         buf.extend_from_slice(&cluster_info.to_be_bytes());
//     }
...
//         ResType::Server => {
//             let three = 3 as u8;
//             buf.extend_from_slice(&three.to_be_bytes());
//         }
//         ResType::None => {
//             let max = 255 as u8;
//             buf.extend_from_slice(&max.to_be_bytes());
